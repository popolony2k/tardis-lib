...
class Serial  {
  ...
  int                               fdDev;
  bool                              bIsOpen;
  string                            strOpenDevice;
  vector<SerialEventListener*>      events;
  pthread_t                         threadId;
  SerialOptions                     serialOptions;



  friend void *__SerialThreadFn( void *arg );
  ...
  bool Open( string strDevice );
  bool Close( void );
  bool IsOpen( void );
  vector<char> Read( int nDataLen, int nBufferLen = FULL_BUFFER ) throw( Exception );
  ssize_t Read( void *pData, int nDataLen, int nBufferLen = FULL_BUFFER ) throw( Exception );
  ssize_t Write( const void *pData, int nDataLen );
  ssize_t Write( vector<char> vData );

  void SetOptions( SerialOptions options );
  SerialOptions &GetOptions( void );
  ...
  void AddSerialEventListener( SerialEventListener *pListener );
  void RemoveSerialEventListener( SerialEventListener *pListener );
  void ReleaseAllListeners( void );
};
